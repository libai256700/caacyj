#!/usr/bin/env python3
"""Validated request boundary for knowledge-query routing and planning."""
...
class RequestEnvelopeError(ValueError):
    """The incoming request cannot be reduced to the public contract."""
...
def _normalize_sources(value: Any) -> tuple[str, ...]:
    if value in (None, ""):
        requested = list(ALLOWED_SOURCES)
    ...
    if not requested or unknown:
        ...
        raise RequestEnvelopeError(
            "invalid_allowed_sources",
            f"allowed_sources contains unsupported values{detail}",
        )
    requested_set = set(requested)
    return tuple(source for source in ALLOWED_SOURCES if source in requested_set)


def _infer_risk(query: str) -> str:
...
def build_request_envelope(
    ...
    *,
    ...
    if embedding is not None and not isinstance(embedding, str):
        raise RequestEnvelopeError("invalid_embedding", "embedding must be a string")

    query, isolation = _normalize_query(payload.get("user_query"), max_query_length)
    requested_risk = _normalize_enum(
        payload.get("risk_level"), "risk_level", RISK_LEVELS, "standard"
    )
    requested_timeliness = _normalize_enum(
        payload.get("timeliness"), "timeliness", TIMELINESS_LEVELS, "stable"
    )
    risk = _max_level(requested_risk, _infer_risk(query), RISK_LEVELS)
    timeliness = _max_level(
        requested_timeliness,
        _infer_timeliness(query),
        TIMELINESS_LEVELS,
    )
    allowed_sources = _normalize_sources(payload.get("allowed_sources"))
    normalized_embedding = (embedding or "").strip().lower()
    isolation.update({
        "transport": transport,
        "requested_embedding": (
            "bge_m3"
        ...
        "risk_escalated": risk != requested_risk,
        "timeliness_escalated": timeliness != requested_timeliness,
    })
    fingerprint_payload = {
    ...
    return RequestEnvelope(
        ...
        isolation=MappingProxyType(isolation),
    )
...
def parse_request_envelope(
    *,
    ...
    user_query = args.get("user_query") or args.get("q")
    return build_request_envelope(
        {
