...
class CloudClaimEvidenceTests(unittest.TestCase):
    ...
    def test_high_risk_claim_requires_single_source_textual_entailment(self):
        unsupported_claims = [
            "飞行前必须检查电池状态，并确认可以夜间飞越人群。",
            "飞行前必须检查电池状态，之后可以在人群上空飞行。",
            "飞行前必须检查电池状态，并确认所有环境下均允许起飞。",
            "飞行前必须检查电池状态，且无需取得许可。",
        ]
        ...
        for unsupported_claim in unsupported_claims:
            with self.subTest(claim=unsupported_claim):
                result = finalize_claim_evidence(
                    unsupported_claim,
                    {"claims": [claim(text=unsupported_claim)]},
                    internal_sources=[source()],
                )
                ...
                self.assertIn(
                    ...
                    result["claims"][0]["reasons"],
                )

    def test_normal_risk_claim_cannot_reverse_or_negate_evidence(self):
        cases = [
            ("该机型不使用锂电池供电。", "该机型使用锂电池供电。"),
            (
                "无人机采用四旋翼布局，但没有稳定性。",
                "无人机采用四旋翼布局，具有良好的稳定性。",
            ),
            ("气象条件不会影响无人机飞行。", "气象条件会影响无人机飞行。"),
        ]

        for unsupported_claim, evidence_text in cases:
            with self.subTest(claim=unsupported_claim):
                result = finalize_claim_evidence(
                    unsupported_claim,
                    {
                        "claims": [
                            claim(text=unsupported_claim, risk="normal")
                        ]
                    },
                    internal_sources=[
                        source(
                            authority="textbook",
                            doc_name="无人机理论书籍_示例.txt",
                            text=evidence_text,
                        )
                    ],
                )
                ...
                self.assertEqual("blocked", result["outcome"])
                self.assertIn(
                    "evidence_not_entailing",
                    result["claims"][0]["reasons"],
                )

    def test_non_exact_question_bank_distractor_cannot_support_model_claim(self):
        unsupported_claim = "飞行前必须关闭设备。"
        result = finalize_claim_evidence(
            unsupported_claim,
            {"claims": [claim(text=unsupported_claim)]},
            internal_sources=[
    ...
    def test_untrusted_high_risk_structured_binding_fails_closed(self):
        answer = "考试条件要求分数达到80分。"
        structured_source = {
            "type": "canonical_csv",
            "file": "exam_conditions.csv",
            "evidence_id": "canonical:exam_conditions:1",
        }

        result = finalize_claim_evidence(
            answer,
            {"claims": [claim(text=answer, evidence=["C1"])]},
        ...
        deterministic = finalize_claim_evidence(
            answer,
            {"claims": [claim(text=answer, evidence=["C1"])]},
            structured_sources=[structured_source],
        ...
        self.assertEqual("passed", deterministic["outcome"])
        self.assertEqual(
            ["canonical:exam_conditions:1"],
            deterministic["telemetry"]["accepted_evidence_ids"],
        )
