...
if __name__ == "__main__":
    sys.stderr.write("formal Cloud 80 CLI bootstrap context is required\n")
    raise SystemExit(2)
...
class CloudEvalBootstrapError(RuntimeError):
    """The Cloud 80 CLI was not entered through its held-byte bootstrap."""


_FORMAL_CLOUD80_SOURCE_STATE_FIELDS = (
    "device",
    "inode",
    "mode",
    "nlink",
...
_EXECUTED_CLOUD80_SOURCE = _open_held_payload(
    Path(__file__),
    "Cloud 80 evaluator source",
    max_bytes=4 * 1024 * 1024,
)
_FORMAL_CLOUD80_SOURCE_SHA256: str | None = None
_FORMAL_CLOUD80_SOURCE_STATE_IDENTITY: dict[str, int] | None = None
_FORMAL_CLOUD80_ACTION_CLOSURE: tuple[object, ...] | None = None


def _cloud80_source_state_identity() -> dict[str, int]:
    state = _EXECUTED_CLOUD80_SOURCE.file_state
    return {
        "device": state.device,
        "inode": state.inode,
        "mode": state.mode,
...
def _current_cloud80_action_closure() -> tuple[object, ...]:
    return (main, _main_impl, _parser)


def _install_formal_cloud80_bootstrap_context(binding: Mapping[str, Any]) -> None:
    global _FORMAL_CLOUD80_ACTION_CLOSURE
    global _FORMAL_CLOUD80_SOURCE_SHA256
    global _FORMAL_CLOUD80_SOURCE_STATE_IDENTITY

    try:
        _require_formal_bootstrap_context()
        _require_isolated_python()
    except OfflineEvidenceError as exc:
        raise CloudEvalBootstrapError(
            "formal Cloud 80 CLI bootstrap context is required"
        ) from exc
    required = {*_FORMAL_CLOUD80_SOURCE_STATE_FIELDS, "sha256"}
    if (
        ...
        or any(
            ...
            for field in _FORMAL_CLOUD80_SOURCE_STATE_FIELDS
        )
        or binding["inode"] == 0
        or binding["nlink"] != 1
        ...
        raise CloudEvalBootstrapError(
            "formal Cloud 80 CLI source binding is malformed"
        )
    expected_state = {
        field: int(binding[field]) for field in _FORMAL_CLOUD80_SOURCE_STATE_FIELDS
    }
    try:
        _revalidate_held_payload(_EXECUTED_CLOUD80_SOURCE)
    except DLPError as exc:
        raise CloudEvalBootstrapError(
            "formal Cloud 80 CLI source binding changed"
        ) from exc
    if (
        hashlib.sha256(_EXECUTED_CLOUD80_SOURCE.payload).hexdigest()
        != binding["sha256"]
        or _cloud80_source_state_identity() != expected_state
    ):
        raise CloudEvalBootstrapError(
            "formal Cloud 80 CLI source differs from held bootstrap bytes"
        )
    _FORMAL_CLOUD80_SOURCE_SHA256 = str(binding["sha256"])
    _FORMAL_CLOUD80_SOURCE_STATE_IDENTITY = expected_state
    _FORMAL_CLOUD80_ACTION_CLOSURE = _current_cloud80_action_closure()


def _require_formal_cloud80_bootstrap_context() -> tuple[object, ...]:
    source_sha256 = _FORMAL_CLOUD80_SOURCE_SHA256
    source_state = _FORMAL_CLOUD80_SOURCE_STATE_IDENTITY
    actions = _FORMAL_CLOUD80_ACTION_CLOSURE
    if (
        not isinstance(source_sha256, str)
        or not _SHA256_RE.fullmatch(source_sha256)
        or not isinstance(source_state, Mapping)
        or set(source_state) != set(_FORMAL_CLOUD80_SOURCE_STATE_FIELDS)
        or any(
    ...
    if (
        hashlib.sha256(_EXECUTED_CLOUD80_SOURCE.payload).hexdigest()
        != source_sha256
        or _cloud80_source_state_identity() != dict(source_state)
        or any(
            current is not expected
            for current, expected in zip(
                _current_cloud80_action_closure(), actions, strict=True
            )
        )
    ):
        raise CloudEvalBootstrapError(
            "formal Cloud 80 CLI source or action closure changed"
        )
    return actions
...
def _main_impl(
    ...
    *,
    _parser_action: object | None = None,
) -> int:
    parser_action = _parser if _parser_action is None else _parser_action
    if not callable(parser_action):
        raise CloudEvalBootstrapError("Cloud 80 CLI parser action is malformed")
...
def main(argv: list[str] | None = None) -> int:
    try:
        actions = _require_formal_cloud80_bootstrap_context()
    except CloudEvalBootstrapError as exc:
        sys.stderr.write(str(exc) + "\n")
        return 2
    return actions[1](argv, _parser_action=actions[2])
