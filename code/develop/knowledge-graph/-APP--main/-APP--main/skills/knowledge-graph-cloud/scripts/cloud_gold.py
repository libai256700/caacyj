...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Prepare a canonical Cloud 80 Gold approval decision for external SSHSIG"
    )
    repo_root = Path(__file__).resolve().parents[3]
    parser.add_argument("--prepare-approval-decision", action="store_true", required=True)
    parser.add_argument(
        "--gold",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        _write_exclusive_bytes(args.output, decision_bytes)
    except (GoldContractError, OSError, ValueError) as exc:
        raise SystemExit(f"Gold approval decision preparation failed: {exc}") from exc
    result = {
        "schema_version": "cloud80-gold-approval-preparation-v1",
        "status": "pending_external_signature",
        "activated": False,
        "decision_path": str(args.output),
        "decision_sha256": sha256_bytes(decision_bytes),
        "signature_identity": SIGNATURE_IDENTITY,
        "signature_namespace": SIGNATURE_NAMESPACE,
    }
