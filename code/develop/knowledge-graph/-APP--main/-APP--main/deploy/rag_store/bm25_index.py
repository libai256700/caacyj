...
from rag_store.anchor_rules import CAL_PRIORITY_RULES, MD92_PRIORITY_RULES, UAS_PRIORITY_RULES, accumulate_chunk_ids
...
# 暂行条例规则已移至 anchor_rules.UAS_PRIORITY_RULES（与 answer_policy 共用唯一事实源，
# 2026-07-12 审计闭环：双份手工维护三次同步改动后抽取）。本表仅保留民航法规则。
...
class BM25Index:
    ...
    def _article_boost_chunk_ids(self, query: str) -> List[str]:
        # UAS 规则在前保持既有插入分优先级（max+20-index 递减）
        return accumulate_chunk_ids(query, UAS_PRIORITY_RULES + CAL_PRIORITY_RULES + MD92_PRIORITY_RULES + CIVIL_LAW_ARTICLE_BOOST_RULES)
