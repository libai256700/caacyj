...
def shadow_backend() -> str:
    return os.getenv("RAG_REMOTE_SHADOW_BACKEND", "planner").strip().lower() or "planner"


def planner_shadow_enabled() -> bool:
    return shadow_enabled() and shadow_backend() == "planner"
...
def select_shadow_domain(question: str, semantic_plan: dict[str, Any] | None, intent: str = "") -> str | None:
    ...
    if planner_shadow_enabled():
        return next((domain for domain in domains if domain != "general"), "general")
...
def _chunk_ids(items: list[dict[str, Any]] | None) -> list[str]:
    return [
...
def build_planner_comparison(
    *,
    ...
    a_merged: list[dict[str, Any]],
    b_merged: list[dict[str, Any]],
    candidate_ids_by_tool: dict[str, list[str]],
    tool_events: list[dict[str, Any]],
    planner_latency_ms: int = 0,
) -> dict[str, Any]:
    """Build a paired A/B retrieval record without generating a B answer."""
    a_ids = _chunk_ids(a_merged)[:20]
    b_ids = _chunk_ids(b_merged)[:20]
    a_rank = {chunk_id: index for index, chunk_id in enumerate(a_ids, start=1)}
    b_rank = {chunk_id: index for index, chunk_id in enumerate(b_ids, start=1)}
    text_ids = set(candidate_ids_by_tool.get("bm25", [])) | set(
        candidate_ids_by_tool.get("dense", [])
    ) | set(candidate_ids_by_tool.get("sqlite_exact", []))
    kg_ids = set(candidate_ids_by_tool.get("neo4j", []))
    kg_unique = sorted(kg_ids - text_ids)
    selected = set(resolution.get("selected_tools") or [])
    executed = {
        str(event.get("tool"))
        for event in tool_events
        if event.get("status") in {"succeeded", "empty", "error"}
    }
    planner_would_skip = sorted(executed - selected)

    kg_rank_changes = []
    missing_rank = max(len(a_ids), len(b_ids), 1) + 1
    for chunk_id in sorted(kg_ids & set(a_ids)):
        before = b_rank.get(chunk_id, missing_rank)
        after = a_rank.get(chunk_id, missing_rank)
        kg_rank_changes.append({
            "chunk_id": chunk_id,
            "without_kg_rank": before,
            "with_kg_rank": after,
            "rank_lift": before - after,
        })
    positive_lifts = [item["rank_lift"] for item in kg_rank_changes if item["rank_lift"] > 0]

    selected_latency_by_tool: dict[str, int] = {}
    for event in tool_events:
        tool = str(event.get("tool") or "")
        if tool in selected and event.get("status") in {"succeeded", "empty", "error"}:
            selected_latency_by_tool[tool] = selected_latency_by_tool.get(tool, 0) + int(
                event.get("latency_ms") or 0
            )
    estimated_b_latency = max(selected_latency_by_tool.values(), default=0) + max(
        int(planner_latency_ms), 0
    )
    overlap_top5 = [chunk_id for chunk_id in a_ids[:5] if chunk_id in set(b_ids[:5])]
    return {
        "pair_valid": True,
        "pair_exclusion_reason": None,
        "comparison_scope": "retrieval_plan_and_ranking_only",
        "answer_quality_comparable": False,
        "a": {
            "role": "production_answer",
            "answer_used": True,
            "top_chunks": a_ids[:5],
            "executed_tools": sorted(executed),
            "latency_ms": local_summary.get("latency_ms"),
        },
        "b": {
            "role": "observe_only_planner",
            "answer_used": False,
            "answer_generated": False,
            "plan": plan,
            "resolution": resolution,
            "top_chunks": b_ids[:5],
            "estimated_latency_ms": estimated_b_latency,
        },
        "tool_selection": {
            "planner_would_skip": planner_would_skip,
            "planner_would_add_but_pool_missing": resolution.get(
                "unresolved_conditional_tools", []
            ),
            "a_tool_events": tool_events,
        },
        "top_k_diff": {
            "a_top5": a_ids[:5],
            "b_top5": b_ids[:5],
            "overlap_top5": overlap_top5,
            "a_only_top5": [chunk_id for chunk_id in a_ids[:5] if chunk_id not in set(b_ids[:5])],
            "b_only_top5": [chunk_id for chunk_id in b_ids[:5] if chunk_id not in set(a_ids[:5])],
        },
        "kg_unique_evidence": {
            "candidate_count": len(kg_unique),
            "candidate_chunk_ids": kg_unique[:20],
            "in_a_top5": [chunk_id for chunk_id in a_ids[:5] if chunk_id in set(kg_unique)],
            "in_b_top5": [chunk_id for chunk_id in b_ids[:5] if chunk_id in set(kg_unique)],
            "graph_path_count": int(local_summary.get("kg_path_count") or 0),
        },
        "kg_rank_lift_proxy": {
            "changes": kg_rank_changes[:20],
            "positive_count": len(positive_lifts),
            "average_positive_lift": (
                round(sum(positive_lifts) / len(positive_lifts), 3)
                if positive_lifts
                else 0.0
            ),
            "ground_truth_metric": False,
        },
        "latency_ms": {
            "a_observed": local_summary.get("latency_ms"),
            "b_estimated_parallel_retrieval": estimated_b_latency,
            "estimate_only": True,
        },
...
def finish_planner_shadow(
    *,
    ...
    """Write an observe-only Planner pair on the existing shadow trace lane."""
    ...
    payload = {
        ...
        "query": query,
        "shadow_mode": True,
        "shadow_backend": "planner",
        "pair_kind": "planner_ab_v1",
