...
public class PostServiceImplTest extends BaseDbUnitTest {
    ...
    @Test
    public void testGetPostPage_orderBySortAsc() {
        PostDO postDO01 = randomPostDO(o -> o.setCode("code_01").setName("name_01").setSort(30));
        postMapper.insert(postDO01);
        PostDO postDO02 = randomPostDO(o -> o.setCode("code_02").setName("name_02").setSort(10));
        postMapper.insert(postDO02);
        PostDO postDO03 = randomPostDO(o -> o.setCode("code_03").setName("name_03").setSort(20));
        postMapper.insert(postDO03);

        PageResult<PostDO> pageResult = postService.getPostPage(new PostPageReqVO());

        assertEquals(3, pageResult.getTotal());
        assertEquals(postDO02.getId(), pageResult.getList().get(0).getId());
        assertEquals(postDO03.getId(), pageResult.getList().get(1).getId());
        assertEquals(postDO01.getId(), pageResult.getList().get(2).getId());
    }
