...
public class SmsCodeServiceImplTest extends BaseDbUnitTest {
    ...
    @Test
    public void sendSmsCode_withinFrequency_success() {
        // 准备参数
        SmsCodeSendReqDTO reqDTO = randomPojo(SmsCodeSendReqDTO.class, o -> {
            o.setMobile("15601691300");
            o.setScene(SmsSceneEnum.MEMBER_LOGIN.getScene());
        ...
        // 每次均创建记录并调用短信发送服务
        SmsCodeDO lastSmsCode = smsCodeMapper.selectLastByMobile(reqDTO.getMobile(), null, null);
        assertEquals(2, lastSmsCode.getTodayIndex());
        verify(smsSendService, times(2)).sendSingleSms(eq(reqDTO.getMobile()), isNull(), isNull(),
                eq("user-sms-login"), eq(MapUtil.of("code", "9999")));
    }
