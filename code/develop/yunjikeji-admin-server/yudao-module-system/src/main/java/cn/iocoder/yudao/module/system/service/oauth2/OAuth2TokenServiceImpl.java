...
public class OAuth2TokenServiceImpl implements OAuth2TokenService {
    ...
    public OAuth2AccessTokenDO checkAccessToken(String accessToken) {
        ...
        fillMissingTenantId(accessTokenDO);
    ...
    private OAuth2AccessTokenDO createOAuth2AccessToken(OAuth2RefreshTokenDO refreshTokenDO, OAuth2ClientDO clientDO) {
        ...
        if (tenantId == null || tenantId <= 0) {
            tenantId = getUserTenantId(refreshTokenDO.getUserId(), refreshTokenDO.getUserType());
        }
    ...
    private OAuth2RefreshTokenDO createOAuth2RefreshToken(Long userId, Integer userType, OAuth2ClientDO clientDO, List<String> scopes) {
        ...
        if (tenantId != null && tenantId > 0) {
            ...
            refreshToken.setTenantId(getUserTenantId(userId, userType));
        }
        oauth2RefreshTokenMapper.insert(refreshToken);
        return refreshToken;
    }

    private void fillMissingTenantId(OAuth2AccessTokenDO accessTokenDO) {
        if (accessTokenDO.getTenantId() != null && accessTokenDO.getTenantId() > 0) {
            return;
        }
        Long tenantId = getUserTenantId(accessTokenDO.getUserId(), accessTokenDO.getUserType());
        if (tenantId == null || tenantId <= 0) {
            return;
        }
        accessTokenDO.setTenantId(tenantId);
        TenantUtils.executeIgnore(() -> oauth2AccessTokenMapper.updateById(accessTokenDO));
        oauth2AccessTokenRedisDAO.set(accessTokenDO);
    }

    private Long getUserTenantId(Long userId, Integer userType) {
