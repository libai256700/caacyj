...
public class TenantServiceImpl implements TenantService {
    ...
    private static final Long ENTERPRISE_AUDIT_DEFAULT_TENANT_PACKAGE_ID = 111L;
    private static final int ENTERPRISE_AUDIT_DEFAULT_ACCOUNT_COUNT = 50;
    private static final String ENTERPRISE_AUDIT_DEFAULT_PASSWORD = "123456.";
    ...
    public Long createTenantByEnterpriseAudit(TenantEnterpriseAuditCreateReqDTO createReqDTO) {
        TenantSaveReqVO normalizedReqVO = BeanUtils.toBean(createReqDTO, TenantSaveReqVO.class);
        normalizedReqVO.setPackageId(ENTERPRISE_AUDIT_DEFAULT_TENANT_PACKAGE_ID);
        normalizedReqVO.setExpireTime(LocalDateTime.now().plusYears(10));
        normalizedReqVO.setAccountCount(ENTERPRISE_AUDIT_DEFAULT_ACCOUNT_COUNT);
        normalizedReqVO.setUsername(buildEnterpriseAuditTenantUsername(normalizedReqVO));
        normalizedReqVO.setPassword(ENTERPRISE_AUDIT_DEFAULT_PASSWORD);
        return createTenant(normalizedReqVO);
    }
    ...
    private String buildEnterpriseAuditTenantUsername(TenantSaveReqVO createReqVO) {
        String mobile = StrUtil.trim(createReqVO.getContactMobile());
        if (StrUtil.isNotBlank(mobile) && mobile.length() >= 4) {
            return "qy" + mobile.substring(mobile.length() - 4);
        }
        return "qy" + System.currentTimeMillis();
    }
