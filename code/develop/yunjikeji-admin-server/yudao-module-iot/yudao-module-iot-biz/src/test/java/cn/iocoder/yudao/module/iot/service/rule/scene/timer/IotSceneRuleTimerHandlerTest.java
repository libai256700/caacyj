...
public class IotSceneRuleTimerHandlerTest {
    ...
    @Test
    public void testRegisterTimerTriggers_schedulerMissing_noop() {
        // 准备参数
        IotSceneRuleTimerHandler handler = new IotSceneRuleTimerHandler();
        IotSceneRuleDO sceneRule = new IotSceneRuleDO();
        sceneRule.setId(5L);
        sceneRule.setStatus(0);
        IotSceneRuleDO.Trigger timerTrigger = new IotSceneRuleDO.Trigger();
        timerTrigger.setType(IotSceneRuleTriggerTypeEnum.TIMER.getType());
        timerTrigger.setCronExpression("0 0 12 * * ?");
        sceneRule.setTriggers(ListUtil.toList(timerTrigger));

        // 调用与验证
        handler.registerTimerTriggers(sceneRule);
    }

    @Test
    public void testUnregisterTimerTriggers_schedulerMissing_noop() {
        // 准备参数
        IotSceneRuleTimerHandler handler = new IotSceneRuleTimerHandler();

        // 调用与验证
        handler.unregisterTimerTriggers(6L);
    }

    @Test
    public void testPauseTimerTriggers_schedulerMissing_noop() {
        // 准备参数
        IotSceneRuleTimerHandler handler = new IotSceneRuleTimerHandler();

        // 调用与验证
        handler.pauseTimerTriggers(7L);
    }
