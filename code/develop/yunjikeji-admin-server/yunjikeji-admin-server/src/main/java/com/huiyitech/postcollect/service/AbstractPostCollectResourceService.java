...
public abstract class AbstractPostCollectResourceService<T extends BaseDO>
        implements PostCollectResourceService {
    ...
    private Map<String, Object> toSnakeCaseMap(T item) {
    ...
    private void applyTenant(T item) {
        if (!(item instanceof TenantBaseDO)) {
            return;
        }
        TenantBaseDO tenantItem = (TenantBaseDO) item;
        if (tenantItem.getTenantId() != null) {
            return;
        }
        Long tenantId = TenantContextHolder.getTenantId();
        tenantItem.setTenantId(tenantId == null ? 0L : tenantId);
    }
    ...
    private Timestamp parseIsoTimestamp(String text) {
        try {
            ...
            // Browser date values may arrive as ISO strings with timezone, for example 2026-06-12T09:30:00.000Z.
            try {
                return Timestamp.from(Instant.parse(text));
            } catch (RuntimeException ignoredAgain) {
                return Timestamp.valueOf(OffsetDateTime.parse(text).atZoneSameInstant(ZoneId.systemDefault()).toLocalDateTime());
            }
        }
    }
