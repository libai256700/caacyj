...
public abstract class AbstractPracticeResourceService<T extends BaseDO> implements PracticeResourceService {
    ...
    private void bindWritableValues(T item, Map<String, Object> body, boolean update) {
        ...
        for (Map.Entry<String, Object> entry : body.entrySet()) {
            ...
            setProperty(item, toPropertyName(column), entry.getValue());
            count++;
        }
        if (count == 0) {
            throw invalidParamException(update ? "Update content cannot be empty" : "Create content cannot be empty");
    ...
    private Map<String, Object> toSnakeCaseMap(T item) {
        ...
        for (PropertyDescriptor descriptor : wrapper.getPropertyDescriptors()) {
            ...
            result.put(toColumnName(name), wrapper.getPropertyValue(name));
        }
        return result;
    }
    ...
    private String toPropertyName(String column) {
        if ("is_correct".equals(column) && writableColumns().contains("is_correct")) {
            return "correct";
        }
    ...
    private String toColumnName(String property) {
        if ("correct".equals(property) && writableColumns().contains("is_correct")) {
            return "is_correct";
        }
