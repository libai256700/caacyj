...
export default defineComponent({
  ...
  setup() {
    ...
    const hasMenuTitle = (item?: AppRouteRecordRaw) => Boolean(item?.meta?.title)

    const getTabMenuItem = (item: AppRouteRecordRaw) => {
      return (
        item.meta?.alwaysShow || (item?.children?.length && item?.children?.length > 1)
          ? item
          : {
    ...
    const tabRouters = computed(() =>
      unref(routers).filter((v) => !v?.meta?.hidden && hasMenuTitle(getTabMenuItem(v)))
    )
