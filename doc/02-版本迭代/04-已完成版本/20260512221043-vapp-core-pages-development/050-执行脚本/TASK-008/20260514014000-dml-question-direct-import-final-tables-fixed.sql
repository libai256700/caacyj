-- TASK-008 direct import into final question bank tables - fixed runtime path
-- Target database: yunjikeji test database only.
-- Persistent write scope: yk_question, yk_question_option.
-- Preconditions:
--   1. 20260513221000-dml-question-category-seed-v2.sql has been executed or confirmed.
--   2. Run 20260514014800-prepare-direct-import-runtime-csv.ps1 to create MySQL-safe TSV files.
...
WHERE x.question_code IS NULL
  ...
  AND EXISTS (
      ...
      WHERE o.question_code = q.question_code
        AND o.option_content <> ''
        AND d.question_code IS NULL
  );

SELECT 'loaded_question_csv_rows' AS metric, COUNT(*) AS metric_value
FROM tmp_task008_direct_question_import;

SELECT 'loaded_option_csv_rows' AS metric, COUNT(*) AS metric_value
FROM tmp_task008_direct_option_import;

SELECT 'duplicate_question_codes' AS metric, COUNT(*) AS metric_value
FROM (
...
SELECT 'duplicate_option_keys' AS metric, COUNT(*) AS metric_value
FROM tmp_task008_duplicate_option_key;

SELECT 'excluded_questions_needs_manual_review_without_option' AS metric, COUNT(*) AS metric_value
FROM tmp_task008_excluded_question;

SELECT 'importable_questions' AS metric, COUNT(*) AS metric_value
FROM tmp_task008_importable_question;

SELECT 'excluded_questions_without_importable_option' AS metric, COUNT(*) AS metric_value
FROM tmp_task008_direct_question_import q
LEFT JOIN tmp_task008_importable_question iq
       ON iq.question_code = q.question_code
LEFT JOIN tmp_task008_excluded_question x
       ON x.question_code = q.question_code
WHERE iq.question_code IS NULL
  AND x.question_code IS NULL
  AND q.question_code <> ''
  AND q.stem <> ''
  AND q.correct_answer <> ''
  AND q.question_type IN ('single_choice', 'multiple_choice', 'judge')
...
SELECT 'importable_options_after_duplicate_key_exclusion' AS metric, COUNT(*) AS metric_value
FROM tmp_task008_direct_option_import o
...
SELECT ROW_COUNT() AS batch_option_deleted_rows;
...
SELECT ROW_COUNT() AS batch_non_importable_question_soft_deleted_rows;
...
SELECT 'final_import_batch_question_count' AS metric, COUNT(*) AS metric_value
FROM yk_question q
JOIN tmp_task008_direct_question_import src
  ON src.question_code = q.question_code
WHERE q.delete_status = 0;

SELECT 'final_total_question_count' AS metric, COUNT(*) AS metric_value
FROM yk_question
WHERE delete_status = 0;

SELECT 'final_import_batch_option_count' AS metric, COUNT(*) AS metric_value
FROM yk_question_option qo
JOIN yk_question q
  ON q.id = qo.question_id
JOIN tmp_task008_direct_question_import src
...
SELECT 'final_total_option_count' AS metric, COUNT(*) AS metric_value
FROM yk_question_option
WHERE delete_status = 0;

SELECT 'final_excluded_question_count' AS metric, COUNT(*) AS metric_value
FROM tmp_task008_excluded_question;

SELECT 'final_duplicate_option_key_count' AS metric, COUNT(*) AS metric_value
FROM tmp_task008_duplicate_option_key;
