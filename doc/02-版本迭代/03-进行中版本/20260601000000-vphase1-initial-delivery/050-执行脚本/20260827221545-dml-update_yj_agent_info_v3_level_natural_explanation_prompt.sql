-- ============================================
-- 脚本类型：dml
-- 脚本描述：统一数据库 V3 system prompt 的确定性仪表盘与正文等级自然解释规则
-- 创建日期：2026-08-27 22:15:45
-- 作者：Codex
-- 影响范围：yj_agent_info.id = 2 且 name = '小题' 的 prompt_config
-- 执行环境：Local 正式记录指向的测试库
-- 回滚脚本：20260827221546-dml-rollback_yj_agent_info_v3_level_natural_explanation_prompt.sql
-- ============================================
...
-- 锁定并展示唯一目标的脱敏前置快照；不得输出完整 prompt_config、reply_strategy 或密钥。
SELECT id,
       name,
       tenant_id,
       status + 0 AS status,
...
SELECT @target_count AS target_count,
       ...
       CASE
           ...
           WHEN @before_is_old = 1 THEN 'READY'
           WHEN @before_is_new = 1 THEN 'ALREADY_APPLIED'
           ELSE 'STOP_UNEXPECTED_PROMPT'
       END AS precheck_result;

-- 只从唯一核验旧值迁移；已是目标值时不更新，重复执行保持幂等。
UPDATE yj_agent_info
SET prompt_config = CONCAT(prompt_config, @prompt_addendum),
    updater = '1',
    update_time = NOW()
WHERE id = 2
  AND name = '小题'
...
SET @postcheck_count = (
    ...
    WHERE id = 2
      ...
      AND INSTR(BINARY prompt_config, BINARY '程序评分仪表盘是系统确定性事实') > 0
      AND INSTR(BINARY prompt_config, BINARY '不得重新计算、修改、质疑或另行判定分数与等级') > 0
      AND INSTR(BINARY prompt_config, BINARY '无需逐字重复官方等级名称') > 0
      AND INSTR(BINARY prompt_config, BINARY '不得出现与系统等级冲突的任何其他官方等级名称') > 0
);
...
SET @execution_pass = IF(
    @target_count = 1
        ...
        AND (
            (@before_is_old = 1 AND @updated_rows = 1)
            OR (@before_is_new = 1 AND @updated_rows = 0)
        ),
    1,
    0
);
...
SELECT @updated_rows AS updated_rows,
       ...
       IF(@execution_pass = 1, 'PASS', 'FAIL_ROLLBACK_REQUIRED') AS execution_result;
...
-- 执行器必须确认 execution_result = PASS；否则立即执行对应回滚脚本并停止后续动作。
