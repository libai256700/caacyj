-- ============================================
-- 脚本类型：dml
-- 脚本描述：在数据库 V3 system prompt 中补齐泛化证照表述约束
-- 创建日期：2026-08-27 20:51:00
-- 作者：Codex
-- 影响范围：yj_agent_info.id = 2 且 name = '小题' 的 prompt_config
-- 执行环境：Local 正式记录指向的测试库
-- 回滚脚本：20260827205101-dml-rollback_yj_agent_info_v3_certificate_prompt.sql
-- ============================================
...
-- 锁定并展示唯一目标的脱敏前置快照；不得输出完整 prompt_config 或 reply_strategy。
SELECT id,
       name,
       tenant_id,
       status + 0 AS status,
...
SET @postcheck_count = (
    ...
    WHERE id = 2
      ...
      AND INSTR(prompt_config, '已有培训或学习基础') > 0
      AND INSTR(prompt_config, '高阶证照') > 0
      AND INSTR(prompt_config, '证照或培训基础') > 0
      AND INSTR(prompt_config, '培训或证照经历') > 0
      AND INSTR(prompt_config, '证照情况') > 0
);
